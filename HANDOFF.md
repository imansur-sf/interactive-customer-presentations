...
Last updated: 2026-08-05 (UX-improvement analysis delivered; a duplicate local checkout was discovered and flagged, unresolved — see "Open items" below). **Read this first — do not re-read prior conversation summaries.** For a detailed history of completed work, see [PROGRESS.md](PROGRESS.md); this file is only current state + what's left.
...
- Manual click-to-edit-text fallback (contenteditable leaf text, bypasses the LLM entirely), icon/image replacement (click-to-upload + chat-URL), slide reordering (drag-to-reorder nav), the Hero KPI countUp-cache fix, and the AI-in-Action/Real-Time-Data desync fix (part A) are all implemented, live-verified, and pushed. See PROGRESS.md for full detail on each.
- A UX-improvement analysis (code-based audit, no live-browser pass — see "Duplicate local checkout" below for why) is complete; findings are in PROGRESS.md's latest entry. Nothing from it has been implemented yet — it's ideas for a future session to triage.
- One item remains open from prior work: animated-slide editability **part B** (new UI surface for JS-only content — explicitly deferred, see item 2 below).
...
3. **Stale `2.0` folder retired** — deleted outright, per user sign-off. It had 324 lines of genuine uncommitted work (old Anthropic/BYOK-era `app.js`/`index.html`/`llm.js` changes, e.g. a `model: 'opus'` reference); backed up to `2.0-folder-uncommitted-backup.patch` before deletion in case anything was worth salvaging, then discarded as no longer relevant once confirmed nothing was worth keeping.
   **Correction (2026-08-05): the "original" (non-`3.0`, no-suffix) folder was NOT actually deleted** — a later session confirmed it still exists on disk at `/Users/imansur/claude/interactive-customer-presentations`. This item's original framing was wrong on that point. See "Duplicate local checkout" under Open items below — it's a stale-but-safe (non-divergent) checkout, not resolved, flagged for a user decision.
...
1. **Duplicate local checkout — needs a user decision, nothing touched.** `/Users/imansur/claude/interactive-customer-presentations` (no `3.0` suffix) still exists on disk and is a second local clone of the same GitHub repo (`imansur-sf/interactive-customer-presentations` — confirmed via `git remote -v` in both folders). It sits at commit `8bd87bb`, which `git merge-base --is-ancestor main origin/main` confirms is a **strict ancestor** of current `origin/main` (`347485a`) — i.e. it's just 74 commits stale, not a divergent/conflicting branch. Its older commits (BYOK/SF-Gateway migration, `Decktools`→`imranAI` rename, GitHub Pages/`.nojekyll`) are legitimate shared history already baked into `3.0`'s current code — they aren't a contradiction, just further back in the same line.
   - **Why it matters**: this session's browser-preview tooling defaults its server `cwd` to this folder (not `3.0`), so its running `icp` preview server was serving this stale, 74-commits-behind snapshot. No live-browser UX verification was performed this session as a result — the UX-improvement analysis below is code/doc-based only, not click-tested.
   - **Why it wasn't just fixed**: the folder has local uncommitted state — modified `.claude/launch.json` and untracked `.claude/session-state.md`/`HANDOFF.md` — and the incoming 74-commit range touches both `.claude/launch.json` and `HANDOFF.md`, so a plain `git pull` would refuse (tracked-file conflict) or silently overwrite (untracked-file collision) rather than cleanly fast-forwarding. Reconciling it wasn't requested, so nothing there was pulled, stashed, or deleted.
   - **Options for next session**: (a) if `3.0` should be the sole copy, delete this folder outright (mirrors what item 3 above already intended before its correction); (b) if it should stay as a live second checkout, back up/discard its 3 dirty files first, then `git pull` (safe fast-forward, confirmed no divergence) so its preview server actually serves current code.

2. **Testing-methodology note**: `preview_click` has intermittently reported success on a target with zero observable effect in past sessions (suspected coordinate/overlap issue) — reconfirmed again this session on `#scope-chip-edit`. This is a caution about the Browser preview MCP tool itself, not an app-code bug — no specific reproducible element was ever pinned down. **Confirmed workaround**: `preview_eval` with `document.getElementById(...).click()` — validated working every time it's been tried.

3. **"AI in Action"/"Real-Time Data" slide editability — part B only remains open.** Part A (make the static HTML the single source of truth for the animation scripts, so editing the static copy no longer silently desyncs from what Play renders) is done — see PROGRESS.md's 2026-08-04/05 entry. Part B is the bigger lift, deliberately deferred:
...
1. Decide how to handle the **duplicate local checkout** (open item 1 above) — delete it or pull it forward. Low urgency but easy to forget, and it silently misdirects browser-preview testing if left as-is.
2. Read the UX-improvement analysis (PROGRESS.md's latest entry) and decide which ideas, if any, to act on. None have been implemented yet.
3. Pick up animated-slide editability **part B** (open item 3 above) when ready to scope a new UI surface — needs a quick design check-in first, not a blind implement.
4. If a new bug report comes in with reason string `css_leak_blocked` or `slide_element_protected`, re-check `llm.js`'s guard chain first since those are the two guards with known false-positive history.
